...
{{
  ...
  "is_known_subject": <bool, only if type=image - CRITICAL RULE: set this to true for ANY request that names a REAL, EXISTING company/brand/product/organization/person/flag/famous landmark - even if the user's wording is 'logo', 'icon', 'emblem', 'belgisi', 'emblemasi' etc. Examples that MUST be true: 'Microsoft Windows logo', 'Windows emblem', 'Apple logo', 'Nike icon', 'Tesla logo', 'Cristiano Ronaldo', 'flag of Uzbekistan', 'Eiffel Tower'. AI image generators CANNOT accurately reproduce exact real trademarks/logos no matter how the prompt is worded - they can only make an artistic impression, which is wrong/low quality for real brands. So whenever a REAL, NAMED entity is requested, ALWAYS prefer searching for the real existing image over generating one. Only set false for original/generic/fictional/purely descriptive requests with NO real-world named entity (e.g. 'a cat in space', 'sunset over mountains', 'a cool icon for my app' with no brand named).>",
