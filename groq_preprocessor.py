...
_SYSTEM_PROMPT = (
    ...
    "4. IMPORTANT style rule: if the request is for a LOGO, ICON, APP ICON, "
    "EMBLEM, BADGE, or similar flat graphic-design asset, you MUST explicitly "
    "add style keywords such as 'flat design, vector style, clean minimal "
    "icon, solid flat colors, no 3D shading, no gradients, plain white "
    "background' - the underlying image model defaults to photorealistic/3D "
    "renders unless told otherwise, which ruins logo/icon requests.\n"
    f"5. The final English prompt MUST be strictly under {GROQ_OUTPUT_MAX_LEN} characters.\n"
